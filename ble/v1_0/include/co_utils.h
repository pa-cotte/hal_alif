...
/// Convert microseconds to half slots - rounded value
#define US_TO_HS_ROUND(us)      ((((us) << 1) + HALF_SLOT_SIZE - 1) / HALF_SLOT_SIZE)
...
/// Convert half microseconds in half slots - truncated value
#define HUS_TO_HS(hus)          ((hus) / HALF_SLOT_SIZE)
 ...
 ****************************************************************************************
 */
#define CLK_DIFF_HUS(int_a, fract_a, int_b, fract_b)      ((CLK_DIFF((int_a), (int_b)) * HALF_SLOT_SIZE) + (fract_b) - (fract_a))
 ...
 ****************************************************************************************
 */
#define CLK_LOWER_EQ_HUS(int_a, fract_a, int_b, fract_b)      ( CLK_DIFF_HUS((int_a), (fract_a), (int_b), (fract_b)) >= 0 )
 ...
 ****************************************************************************************
 */
#define CLK_GREATER_THAN_HUS(int_a, fract_a, int_b, fract_b)      ( CLK_DIFF_HUS((int_a), (fract_a), (int_b), (fract_b)) < 0 )
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint32_t co_read24(void const *ptr24)
{
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE void co_write24(void const *ptr24, uint32_t value)
{
...
__STATIC_FORCEINLINE uint32_t co_read32(void const *ptr32)
{
    ...
    addr_h = co_read16((uint8_t *)ptr32 + 2);
    return ((uint32_t)addr_l | (uint32_t)addr_h << 16);
}
/**
 ****************************************************************************************
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t co_bit_cnt(const uint8_t* p_val, uint8_t size)
{
    uint8_t nb_bit = 0;
    while(size-- > 0)
    {
 ...
 *
 * @param[in/out] clock_hs   Value of the clock (in half-slot)
 * @param[in/out] clock_hus  Value of the clock (in half-us)
 * @param[in]     shift      Distance of the shift (in half-us)
 ****************************************************************************************
 */
__STATIC_FORCEINLINE void co_util_clk_shift(uint32_t* clock_hs, uint16_t* clock_hus, int32_t shift)
{
    shift += *clock_hus;
    ...
    if (shift < 0)
    {
        ...
        *clock_hus = shift + num_hs*HALF_SLOT_SIZE;
        *clock_hs = CLK_SUB(*clock_hs, num_hs);
    }
    else
    {
        uint16_t num_hs = shift/HALF_SLOT_SIZE;
        *clock_hus = shift - num_hs*HALF_SLOT_SIZE;
        *clock_hs = CLK_ADD(*clock_hs, num_hs);
    }
}
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE void co_util_clk_shift_forward(uint32_t* clock_hs, uint16_t* clock_hus, uint32_t shift)
{
    uint16_t num_hs;
    shift += *clock_hus;
 ...
 *****************************************************************************************
 */
__STATIC_FORCEINLINE uint16_t co_strlen(const char* p_str)
{
