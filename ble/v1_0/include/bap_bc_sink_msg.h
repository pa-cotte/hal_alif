...
typedef struct bap_bc_sink_enable_req_ind
{
    ...
    uint8_t con_lid;
    /// Stream position bit field indicating Streams to synchronize with or Streams available for synchronization
    /// if No Preference policy has been set by Assistant device
    uint32_t stream_pos_bf;
...
typedef struct bap_bc_sink_enable_cfm
{
    ...
    /// Status
    uint16_t status;
    /// Application choice of sink streams to synchronize with (Used when Bis sync bf is set to no pref)
    uint32_t stream_sync_bf;
    /// Group local index
    uint8_t grp_lid;
    /// Timeout duration (in multiple of 10ms) before considering synchronization has been lost
    /// From 100ms to 163.84s
