...
enum gapi_msg_ids
{
    /* Default event */
    /// Command
    GAPI_CMD = MSG_ID(GAPI, 0x00),
    /// Command Complete event
    GAPI_CMP_EVT = MSG_ID(GAPI, 0x01),
    /// Request
    GAPI_REQ = MSG_ID(GAPI, 0x02),
    /// Response
    GAPI_RSP = MSG_ID(GAPI, 0x03),
    /// Indication
    GAPI_IND = MSG_ID(GAPI, 0x04),
    /// Request indication
    GAPI_REQ_IND = MSG_ID(GAPI, 0x05),
    /// Confurmation
    GAPI_CFM = MSG_ID(GAPI, 0x06),
};
...
enum gapi_msg_req_code
{
    ...
    GAPI_INTF_REGISTER = GAPI_CODE(COMMON, 0),

    /// Unicast - Add Group
    GAPI_UG_ADD = GAPI_CODE(UG, 0),
    /// Unicast - Add Group with Test Parameters
    GAPI_UG_TEST_ADD = GAPI_CODE(UG, 1),
    /// Unicast - Configure Stream
    GAPI_US_CONFIG = GAPI_CODE(UG, 2),
    /// Unicast - Configure Stream with Test Parameters
    GAPI_US_TEST_CONFIG = GAPI_CODE(UG, 3),
    /// Unicast - Bind Stream with a Connection
    GAPI_US_BIND = GAPI_CODE(UG, 5),
    /// Unicast - Release Stream
    GAPI_US_RELEASE = GAPI_CODE(UG, 6),

    /// Broadcast - Add Group
    GAPI_BG_ADD = GAPI_CODE(BG, 0),
    /// Broadcast - Add Group with Test Parameters
    GAPI_BG_TEST_ADD = GAPI_CODE(BG, 1),

    /// Data Path - Enable/disable ISO over HCI
    GAPI_DP_ENABLE_ISOOHCI = GAPI_CODE(DP, 0),
    /// Data Path - Setup
    GAPI_DP_SETUP = GAPI_CODE(DP, 1),
    /// Data Path - Remove
    GAPI_DP_REMOVE = GAPI_CODE(DP, 2),

    /// Debug - Get Features
    GAPI_DBG_FEATURES = GAPI_CODE(DBG, 0),
};
...
enum gapi_msg_ind_code
{
    ...
    GAPI_UNKNOWN_MSG = GAPI_CODE(COMMON, 0),

    /// Unicast - Stream Enabled
    GAPI_US_ENABLED = GAPI_CODE(UG, 0),
    /// Unicast - Stream Disabled
    GAPI_US_DISABLED = GAPI_CODE(UG, 1),

    /// Broadcast - Synchronization Status
    GAPI_BG_SYNC_STATUS = GAPI_CODE(BG, 0),
    /// Broadcast - Group Added for synchronization
    GAPI_BG_SYNC_ADDED = GAPI_CODE(BG, 1),
    /// Broadcast - Group Created
    GAPI_BG_CREATED = GAPI_CODE(BG, 2),

    /// Data Path - Update
    GAPI_DP_UPDATE = GAPI_CODE(DP, 0),
    /// Data Path - ISO data received
    GAPI_DP_DATA_RX = GAPI_CODE(DP, 1),

    /// Test Mode - Counters
    GAPI_TM_CNT = GAPI_CODE(TM, 0),

    #if (AUDIO_SYNC_SUPPORT && BLE_BIS)
    /// AUSY mode
    GAPI_AUSY_ESTAB_BIS = GAPI_CODE(AUSY, 0),
...
enum gapi_msg_req_ind_code
{
    ...
    GAPI_US_ESTABLISH = GAPI_CODE(UG, 0),
};
...
typedef struct gapi_ug_add_req
{
...
typedef struct gapi_ug_add_rsp
{
    /// Request code (shall be set to #GAPI_UG_ADD)
    uint16_t req_code;
    /// Status
...
typedef struct gapi_us_config_req
{
...
typedef struct gapi_us_config_rsp
{
    /// Request code (shall be set to #GAPI_US_CONFIG)
    uint16_t req_code;
    /// Status
...
typedef struct gapi_us_establish_req_ind
{
...
typedef struct gapi_us_establish_cfm
{
    /// Request indication code (shall be set to #GAPI_US_ESTABLISH)
    uint16_t req_ind_code;
    /// Status
...
typedef struct gapi_us_bind_req
{
...
typedef struct gapi_ug_update_cmd
{
...
typedef struct gapi_us_enable_cmd
{
...
typedef struct gapi_us_disable_cmd
{
...
typedef struct gapi_us_release_req
{
...
typedef struct gapi_ug_remove_cmd
{
...
typedef struct gapi_bg_add_req
{
...
typedef struct gapi_bg_add_rsp
{
    /// Request code (shall be set to #GAPI_BG_ADD)
    uint16_t req_code;
    /// Status
...
typedef struct gapi_bg_enable_cmd
{
...
typedef struct gapi_bg_sync_cmd
{
...
typedef struct gapi_bg_sync_added_ind
{
...
typedef struct gapi_bg_disable_cmd
{
...
typedef struct gapi_bg_remove_cmd
{
...
typedef struct gapi_ug_test_add_req
{
...
typedef struct gapi_us_test_config_req
{
...
typedef struct gapi_bg_test_add_req
{
...
typedef struct gapi_dp_enable_isoohci_req
{
...
typedef struct gapi_dp_setup_req
{
    ...
    /// Direction
    uint8_t direction;
    /// Data path ID (see #gapi_dp enumeration)
    uint8_t dp_id;
...
typedef struct gapi_dp_remove_req
{
...
typedef struct gapi_tm_start_cmd
{
...
typedef struct gapi_tm_cnt_get_cmd
{
...
typedef struct gapi_tm_stop_cmd
{
...
typedef struct gapi_get_quality_cmd
{
...
typedef struct gapi_get_quality_cmp_evt
{
    /// Command code (shall be set to #GAPI_GET_QUALITY)
    uint16_t cmd_code;
    /// Status
...
typedef struct gapi_register_req
{
...
typedef struct gapi_us_disabled_ind
{
...
typedef struct gapi_us_enabled_ind
{
...
typedef struct gapi_bg_sync_status_ind
{
...
typedef struct gapi_bg_created_ind
{
...
typedef struct gapi_dp_update_ind
{
...
typedef struct gapi_dp_data_rx_ind
{
...
typedef struct gapi_tm_cnt_ind
{
...
typedef struct gapi_dbg_features_rsp
{
...
typedef struct gapi_unknown_msg_ind
{
