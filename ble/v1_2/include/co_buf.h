 ...
 *
 * If total length is lower than CO_BUF_SMALL_SIZE and small buffer pool is not empty:
 *      A buffer will be picked from small buffer pool.
 * else if total length is lower than CO_BUF_BIG_SIZE and big buffer pool is not empty:
 ...
 * When doing a buffer allocation, acquisition counter is equals to 1.
 * Buffer isn't free by #co_buf_release function when acquisition counter is equals to 0.
 ...
 *            if a free callback has been configured using #co_buf_cb_free_set:
 ...
 *
 *            else if a buffer has been initialized with #co_buf_prepare:
 *                  nothing is done
 *
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t* co_buf_data(co_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint16_t co_buf_data_len(const co_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint16_t co_buf_head_len(const co_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t* co_buf_head(co_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t* co_buf_tail(co_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint16_t co_buf_tail_len(const co_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 *        length is less or equals to tail length.
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t* co_buf_metadata(co_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;

    if((p_buf != NULL) && (GETF(p_buf->metadata_bf, CO_BUF_METADATA_FROZEN_SIZE) < CO_BUF_META_DATA_SIZE))
    {
 ...
 *        pointer given by #co_buf_metadata function .
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE uint8_t co_buf_metadata_len(const co_buf_t *p_buf)
{
    uint8_t ret = 0;
 ...
 *
 *        @note co_buf_alloc function is used to allocate output buffer.
 *
