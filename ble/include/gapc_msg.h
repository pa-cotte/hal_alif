...
enum gapc_msg_id
{
    ...
    /// \endif
    GAPC_BT_PERIPH_CONNECTION_ESTABLISHED_IND = MSG_ID(GAPC, 0x08),
    /// Indicate that there is no more ATT bearer available
    /// see #gapc_connection_info_cb.no_more_att_bearer
    GAPC_NO_MORE_ATT_BEARER_IND = MSG_ID(GAPC, 0x04),
    ...
    GAPC_LE_PEER_FEATURES_IND = MSG_ID(GAPC, 0x13),
    /// Indication of ongoing connection RSSI
    GAPC_RSSI_IND = MSG_ID(GAPC, 0x14),
    /// Indication of ongoing connection Channel Map
    GAPC_LE_CHANNEL_MAP_IND = MSG_ID(GAPC, 0x15),
    ...
    GAPC_LE_SET_PREFERRED_PERIPH_LATENCY_CMD = MSG_ID(GAPC, 0x4B),
    /// Set the preferred slave event duration (for slave only, with RW controller)
    GAPC_LE_SET_PREFERRED_PERIPH_EVENT_DURATION_CMD = MSG_ID(GAPC, 0x4C),
    /// Set the maximum reception and size and time using DLE negotiation
    GAPC_LE_SET_MAX_RX_SIZE_AND_TIME_CMD = MSG_ID(GAPC, 0x4D),
    /// Request update of the subrating parameters applied on a connection
    GAPC_LE_UPDATE_SUBRATE_CMD = MSG_ID(GAPC, 0x4E),
    /// Indicate subrating parameters applied on a given connection
    GAPC_LE_SUBRATE_IND = MSG_ID(GAPC, 0x4F),
    /// Set priority elevation level
    GAPC_SET_PRIORITY_ELEVATION_CMD = MSG_ID(GAPC, 0x90),
    /// Change supported packet types command
    GAPC_BT_CHANGE_PACKET_TYPE_CMD = MSG_ID(GAPC, 0x91),
    /// Supported packet types changed indication
    GAPC_BT_PACKET_TYPE_CHANGED_IND = MSG_ID(GAPC, 0x92),
    /// Switch role command
    GAPC_BT_SWITCH_ROLE_CMD = MSG_ID(GAPC, 0x93),
    /// Role changed indication
    GAPC_BT_ROLE_CHANGED_IND = MSG_ID(GAPC, 0x94),
    /// Configure Sniff Mode command
    GAPC_BT_CONFIGURE_SNIFF_MODE_CMD = MSG_ID(GAPC, 0x95),
    /// Mode changed indication
    GAPC_BT_MODE_CHANGED_IND = MSG_ID(GAPC, 0x96),
    /// Setup QoS command
    GAPC_BT_SETUP_QOS_CMD = MSG_ID(GAPC, 0x97),
    /// QoS Setup indication
    GAPC_BT_QOS_SETUP_IND = MSG_ID(GAPC, 0x98),
    /// Set Tx Power command
    GAPC_LE_SET_TX_POWER_CMD = MSG_ID(GAPC, 0x99),
    /// Set Tx Power indication
    GAPC_LE_SET_TX_POWER_IND = MSG_ID(GAPC, 0x9A),
    /// Write link policy settings command
    GAPC_BT_WRITE_LINK_POLICY_SETTINGS_CMD = MSG_ID(GAPC, 0x9B),

    /* Bonding procedure */
    /// Start Bonding command procedure
    GAPC_LE_BOND_CMD = MSG_ID(GAPC, 0x50),
    ...
    GAPC_BT_SET_REQUIRED_SECURITY_LEVEL_CMD = MSG_ID(GAPC, 0x5D),

    /* Periodic Sync Transfer */
    /// Transfer periodic advertising sync information to peer device
    GAPC_LE_START_PAST_CMD = MSG_ID(GAPC, 0x60),

    /* Client Features */
    /// Enable usage of supported client features
    GAPC_LE_ENABLE_CENTRAL_FEATURES_CMD = MSG_ID(GAPC, 0x61),


    /* Constant Tone Extension */
    /// Constant Tone Extension Transmission configuration command
    GAPC_LE_CONFIGURE_CTE_TX_CMD = MSG_ID(GAPC, 0x70),
    /// Constant Tone Extension Reception configuration command
    GAPC_LE_CONFIGURE_CTE_RX_CMD = MSG_ID(GAPC, 0x71),
    /// Constant Tone Extension request control command (enable / disable)
    GAPC_LE_CONTROL_CTE_REQUEST_CMD = MSG_ID(GAPC, 0x72),
    /// Constant Tone Extension Response control command (enable / disable)
    GAPC_LE_CONTROL_CTE_RESPONSE_CMD = MSG_ID(GAPC, 0x73),
    /// Indicate reception of a IQ Report event over a ble connection
    GAPC_LE_CTE_IQ_REPORT_IND = MSG_ID(GAPC, 0x74),
    /// Indicate that an IQ Request has been rejected or CTE data not present in LMP response
    GAPC_LE_CTE_REQUEST_FAILED_IND = MSG_ID(GAPC, 0x75),

    /* LE Power Control */
    /// Local TX power indication
    GAPC_LE_LOCAL_TX_POWER_IND = MSG_ID(GAPC, 0x80),
    /// Remote TX power indication
    GAPC_LE_PEER_TX_POWER_IND = MSG_ID(GAPC, 0x81),
    /// Control TX Power Reports command
    GAPC_LE_CONTROL_TX_POWER_REPORT_CMD = MSG_ID(GAPC, 0x82),
    /// Local TX power change report indication
    GAPC_LE_LOCAL_TX_POWER_REPORT_IND = MSG_ID(GAPC, 0x83),
    /// Remote TX power change report indication
    GAPC_LE_PEER_TX_POWER_REPORT_IND = MSG_ID(GAPC, 0x84),
    /// Control Path loss configuration
    GAPC_LE_CONTROL_PASS_LOSS_CMD = MSG_ID(GAPC, 0x85),
    /// Path Loss Threshold Event Indication
    GAPC_LE_PATH_LOSS_THRESHOLD_IND = MSG_ID(GAPC, 0x86),
};
/// @} GAPC_MSG_ID_API
...
enum gapc_operation
{
    ...
    GAPC_LE_REQUEST_SECURITY = 0x52,
    /// Request to inform the remote device when keys have been entered or erased
    GAPC_LE_NOTIFY_KEY_PRESSED = 0x53,
    /// Set BT classic connection required security level operation
    GAPC_BT_SET_REQUIRED_SECURITY_LEVEL = 0x54,

    #if (BLE_PAST)
    /* Periodic Sync Transfer */
    /// Transfer periodic advertising sync information to peer device
    GAPC_LE_START_PAST = 0x60,
    #endif // (BLE_PAST)

    #if (!0)
    /* Client Features */
    /// Enable usage of supported client features
    GAPC_LE_ENABLE_CENTRAL_FEATURES = 0x61,
    #endif // (!0)

    /* Constant Tone Extension */
    /// Constant Tone Extension Transmission configuration
    GAPC_LE_CONFIGURE_CTE_TX = 0x70,
    /// Constant Tone Extension Reception configuration
    GAPC_LE_CONFIGURE_CTE_RX = 0x71,
    /// Constant Tone Extension request control (enable / disable)
    GAPC_LE_CONTROL_CTE_REQUEST = 0x72,
    /// Constant Tone Extension Response control (enable / disable)
    GAPC_LE_CONTROL_CTE_RESPONSE = 0x73,
...
enum gapc_dev_info
{
    ...
    GAPC_DEV_NAME = 0,
    /// Device Appearance
    GAPC_DEV_APPEARANCE = 1,
    /// Device Slave preferred parameters
    GAPC_DEV_SLV_PREF_PARAMS = 2,
    /// Device Central address resolution
    GAPC_DEV_CTL_ADDR_RESOL = 3,
    /// Device database hash value
    GAPC_DEV_DB_HASH = 4,
    /// Resolvable Private address only after bond
    GAPC_DEV_RSLV_PRIV_ADDR_ONLY = 5,
    /// Client supported features
    GAPC_DEV_CLI_SUP_FEAT = 8,
    /// Server supported features
    GAPC_DEV_SRV_SUP_FEAT = 9,
...
typedef struct gapc_operation_cmd
{
    /// Connection index
    uint8_t conidx;
    /// GAP request type
...
/*@TRACE*/
typedef struct gapc_unknown_msg_ind
{
    /// Connection index
    uint8_t conidx;
    /// Unknown message id
...
/// Set specific link data configuration.
/*@TRACE*/
typedef struct gapc_connection_cfm
{
    /// Connection index
    uint8_t conidx;
    /// Bond data
...
/*@TRACE*/
typedef struct gapc_no_more_att_bearer_ind
{
    /// Connection index
    uint8_t conidx;
...
/// Request disconnection of current link command.
/*@TRACE*/
typedef struct gapc_disconnect_cmd
{
    /// Connection index
    uint8_t conidx;
    /// GAP request type:
...
/// Indicate that a link has been disconnected
/*@TRACE*/
typedef struct gapc_disconnect_ind
{
    /// Connection index
    uint8_t conidx;
    /// Connection handle
...
/// Retrieve information command
/*@TRACE*/
typedef struct gapc_get_info_cmd
{
    /// Connection index
    uint8_t conidx;
    /// GAP request type:
...
/*@TRACE*/
typedef struct gapc_peer_att_info_ind
{
    /// Connection index
    uint8_t conidx;
...
/// Indication of peer version info
/*@TRACE*/
typedef struct gapc_peer_version_ind
{
    /// Connection index
    uint8_t conidx;
    /// Manufacturer name
...
/*@TRACE*/
typedef struct gapc_ping_to_value_ind
{
    /// Connection index
    uint8_t conidx;
...
/*@TRACE*/
typedef struct gapc_get_info_req_ind
{
    /// Connection index
    uint8_t conidx;
    /// Requested information (see #gapc_dev_info enumeration)
...
/*@TRACE*/
typedef struct gapc_get_info_cfm
{
    /// Connection index
    uint8_t conidx;
    /// Requested information (see #gapc_dev_info enumeration)
...
/*@TRACE*/
typedef struct gapc_set_info_req_ind
{
    /// Connection index
    uint8_t conidx;
    /// Requested information (see #gapc_dev_info enumeration)
...
/*@TRACE*/
typedef struct gapc_set_info_cfm
{
    /// Connection index
    uint8_t conidx;
    /// Requested information (see #gapc_dev_info enumeration)
...
/// Bonding requested by peer device indication message.
/*@TRACE*/
typedef struct gapc_bond_req_ind
{
    /// Connection index
    uint8_t conidx;
...
/*@TRACE*/
typedef struct gapc_bond_data_update_ind
{
    /// Connection index
    uint8_t conidx;
    /// Updated bond data
...
/*@TRACE*/
typedef struct gapc_set_ping_to_cmd
{
    /// Connection index
    uint8_t conidx;
    /// GAP request type:
...
/*@TRACE*/
typedef struct gapc_ping_to_ind
{
    /// Connection index
    uint8_t conidx;
} gapc_ping_to_ind_t;

/// Parameters of the @ref GAPC_SET_PRIORITY_ELEVATION_CMD message
/*@TRACE*/
typedef struct gapc_set_priority_elevation_cmd
{
    /// Connection index
    uint8_t conidx;
    /// GAP request type:
    /// - #GAPC_SET_PRIORITY_ELEVATION: Set priority elevation level
    uint8_t operation;
    /// Priority elevation (in units of increment)
    uint8_t elevation;
} gapc_set_priority_elevation_cmd_t;
