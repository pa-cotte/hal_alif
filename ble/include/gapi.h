...
#define GAPI_BROADCAST_CODE_LEN         (16)
...
#define GAPI_CODE(module, idx)          ((GAPI_MODULE_ ## module << 8) | idx)
/// Retrieve module from message code
#define GAPI_MODULE(code)               ((code & 0x0F00) >> 8)
/// Retrieve code index from message code
#define GAPI_CODE_IDX(code)             (code & 0x00FF)
...
{
    /// Get Quality
    GAPI_CMD_TYPE_GET_QUALITY = 0,

    /// Unicast - Update Group
    GAPI_CMD_TYPE_UG_UPDATE = 0,
    /// Unicast - Enable Stream
    GAPI_CMD_TYPE_US_ENABLE,
    /// Unicast - Disable Stream
    GAPI_CMD_TYPE_US_DISABLE,
    /// Unicast - Remove Group
    GAPI_CMD_TYPE_UG_REMOVE,

    /// Broadcast - Enable Group
    GAPI_CMD_TYPE_BG_ENABLE = 0,
    /// Broadcast - Synchronize with Group
    GAPI_CMD_TYPE_BG_SYNC,
    /// Broadcast - Disable Group
    GAPI_CMD_TYPE_BG_DISABLE,
    /// Broadcast - Remove Group
    GAPI_CMD_TYPE_BG_REMOVE,

    /// Test Mode - Start
    GAPI_CMD_TYPE_TM_START = 0,
    /// Test Mode - Get Counters
    GAPI_CMD_TYPE_TM_CNT_GET,
    /// Test Mode - Stop
    GAPI_CMD_TYPE_TM_STOP,

    /// AUSY - Start
    GAPI_CMD_TYPE_AUSY_CONFIG_BIS = 0,
...
{
    /// u-Law log
    GAPI_CODEC_FORMAT_U_LAW = 0,
    /// A-Lay log
    GAPI_CODEC_FORMAT_A_LAW,
    /// CVSD
    GAPI_CODEC_FORMAT_CVSD,
    /// Transparent (controller does not do any transcoding or resampling)
    GAPI_CODEC_FORMAT_TRANSPARENT,
    /// Linear PCM
    GAPI_CODEC_FORMAT_LINEAR_PCM,
    /// mSBC
    GAPI_CODEC_FORMAT_MSBC,
    /// LC3
    GAPI_CODEC_FORMAT_LC3,

    /// Maximum Codec Format
    GAPI_CODEC_FORMAT_SIG_MAX,

    /// Vendor Specific Codec
    GAPI_CODEC_FORMAT_VENDOR = 0xFF,
};
...
{
    /// Input data path
    GAPI_DP_DIRECTION_IN_POS = GAPI_DP_DIRECTION_INPUT,
    GAPI_DP_DIRECTION_IN_BIT = CO_BIT(GAPI_DP_DIRECTION_IN_POS),

    /// Output data path
    GAPI_DP_DIRECTION_OUT_POS = GAPI_DP_DIRECTION_OUTPUT,
...
enum gapi_bg_sync_status
{
    ...
    GAPI_BG_SYNC_STATUS_ESTABLISHED = 0,
...
typedef struct gapi_us_bind_info
{
    /// Stream local index
    uint8_t stream_lid;
    /// Connection index
...
typedef struct gapi_ug_param_common
{
...
typedef struct gapi_ug_test_param
{
    /// Common information
    gapi_ug_param_common_t common;
...
typedef struct gapi_us_param_common
{
...
typedef struct gapi_us_test_param
{
    /// Common information
    gapi_us_param_common_t common;
...
typedef struct gapi_bg_param_common
{
...
typedef struct gapi_bg_test_param
{
    /// Common information
    gapi_bg_param_common_t common_param;
...
typedef struct gapi_bg_sync_config
{
...
typedef struct gapi_dp_codec_cfg
{
    /// Codec ID
...
typedef struct gapi_ausy_config_bis
{
...
typedef struct gapi_dp_codec_cfg_single
{
    /// Codec ID
    uint8_t codec_id[CODEC_ID_LEN];
    /// Number of Codec Configuration pieces
...
typedef struct gapi_cbs_isoohci
{
    /**
     ****************************************************************************************
 ...
 ****************************************************************************************
 */
uint16_t gapi_ug_test_add(gapi_ug_test_param_t *p_group_info, uint8_t cig_id, uint8_t intf_lid,
                         uint8_t* p_group_lid);
/**
 ****************************************************************************************
 ...
 ****************************************************************************************
 */
uint16_t gapi_sm_stream_conhdl_get_with_id(uint8_t group_id, uint8_t stream_id);
